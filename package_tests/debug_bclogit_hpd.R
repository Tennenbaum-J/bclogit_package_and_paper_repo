...
cat("\n--------------------------\n")
cat("Running prior: PMP, concordant_fit: GLM\n")
fit <- tryCatch(
    {
        bclogit::bclogit(
            y = y,
...
if (!is.null(fit) && !is.null(fit$model)) {
    cat("GLM fit worked! beta_hat_T:", fit$coefficients[1], "\n")

    cat("Running prior: PMP, concordant_fit: GEE\n")
    fit2 <- tryCatch(
        {
            bclogit::bclogit(
                y = y,
        ...
        error = function(e) {
            cat("GEE failed with error:", e$message, "\n")
            NULL
        }
    )

    cat("Running prior: PMP, concordant_fit: GLMM\n")
    fit3 <- tryCatch(
        {
            bclogit::bclogit(
                y = y,
        ...
        error = function(e) {
            cat("GLMM failed with error:", e$message, "\n")
            NULL
        }
    )
}
